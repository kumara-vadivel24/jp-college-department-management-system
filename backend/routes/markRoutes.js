...
// GET /api/marks/semester - Fetch Semester Exam Marks
...
// POST /api/marks/semester/update - Record / Update Semester Exam Marks
router.post('/semester/update', authenticateToken, authorizeRoles('hod', 'faculty'), async (req, res) => {
  try {
...
// POST /api/marks/update - Update internal marks
router.post('/update', authenticateToken, authorizeRoles('hod', 'faculty'), async (req, res) => {
  try {
        ...
        if (mlRes.data) {
          ...
          await db.runAsync(
            `INSERT INTO predictions (student_id, exam_term, predicted_result, confidence_score, pass_probability, risk_level, recommended_action, focus_areas, model_version)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [student_id, 'Internal Cumulative', predicted_result, confidence_score, pass_probability, risk_level, recommended_action, subject_focus_areas.join(', '), model_version]
          );
        }
      } catch (mlErr) {}
    }
...
// GET /api/marks/export/excel - Export Marks sheet to Excel (.xlsx)
router.get('/export/excel', authenticateToken, authorizeRoles('hod', 'faculty'), async (req, res) => {
  try {
    const { type } = req.query; // 'internal' or 'semester'
    let data = [];
...
// GET /api/marks/courses - Get courses list
